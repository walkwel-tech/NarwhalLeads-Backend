...
export const activityLogs = async () => {
  cron.schedule("*/10 * * * *", async () => {
    ...
    const userDataPromises: Array<Promise<UserPromise>> = activity.map(async (activity: ActivityLogsInterface) => {
      const user: UserInterface = await User.findOne({_id:activity.userEntity,isSignUpCompleteWithCredit:true}).populate(
        "businessDetailsId"
      ) ?? {} as UserInterface;

      const userBusiness: BusinessDetailsInterface | null = isBusinessObject(user?.businessDetailsId) 
