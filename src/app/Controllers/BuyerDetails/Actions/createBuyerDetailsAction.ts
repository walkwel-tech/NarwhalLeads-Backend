...
export const createBuyerQuestions = async (
  buyerQuestions: BuyerQuestionInput[],
  clientId: any
): Promise<any> => {
  if (
    !buyerQuestions ||
    !Array.isArray(buyerQuestions) ||
    buyerQuestions.length === 0
  ) {
    return {
      error: "Buyer questions array is required and should not be empty",
    };
  }
